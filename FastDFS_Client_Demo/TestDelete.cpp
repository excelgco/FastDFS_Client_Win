...
static TestFileInfo files[FILE_TYPE_COUNT] = {
	{5 * 1024, "5K",         1, 0, 0, 0},
	{50 * 1024, "50K",       0, 0, 0, 0}, 
	{200 * 1024, "200K",     0, 0, 0, 0},
	{1 * 1024 * 1024, "1M",  0, 0, 0, 0},
	{10 * 1024 * 1024, "10M",  0, 0, 0, 0},
	{100 * 1024 * 1024, "100M", 0, 0, 0, 0}
};
*/
...
{	
	...
	files[0].count = 2;
	files[0].delete_count = 0;
	files[0].success_count = 0;
	files[0].time_used = 0;
}

TestDelete::TestDelete(int index, int file_num)
{	
	storage_count = 0;
	total_count = 0;
	success_count = 0;
...
int TestDelete::save_stats_by_file_type()
{
	int k;
	char filename[64];
	FILE *fp;

	sprintf(filename, "c:\\delete\\%s.%d", STAT_FILENAME_BY_FILE_TYPE, proccess_index);
...
int TestDelete::save_stats_by_overall()
{
	char filename[64];
	FILE *fp;

	sprintf(filename, "c:\\delete\\%s.%d", STAT_FILENAME_BY_OVERALL, proccess_index);
	if ((fp=fopen(filename, "wb")) == NULL)
	{
		char debugBuf[256];
		sprintf(debugBuf, "open file %s fail, errno: %d, error info: %d\n", filename, errno, errno);
...
int TestDelete::get_file_type_index(const int file_bytes)
{
	TestDeleteFileInfo *pFile;
	TestDeleteFileInfo *pEnd;
...
int TestDelete::getFileContent(const char *filename, char **buff, UINT64 *pfile_size)
{
